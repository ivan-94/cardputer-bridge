...
- App 重启恢复不再信任设备遗留的 `audio:ready`：只有当前进程收到通过认证的 UDP 包才算 session 就绪，否则自动协商新 session。新 App 进程总是以 muted 为意图基线；真机重启 1538–1988ms 内设备回到 `muted/closed`，独立系统 PCM 的 active/tail peak 均为 0。Cardputer 端发起的 live 意图也能跨过 App heartbeat 保持，不会被旧默认值覆盖。生产更新能力纳入后 macOS 71/71 tests 通过。
...
- `docs/evidence/FF-7-USB-migration-0.10.0-2026-08-29.md` — `0.10.0` 分区迁移、四区 Flash verify、NVS 保留、健康确认与升级后主链路实机证据。
...
- 2026-08-29 `0.10.0` 生产更新候选：macOS 71/71 tests、73 项 contract、签名发布清单、Ed25519 篡改拒绝、USB 安全顺序、版本防降级、双 OTA 分区、HTTPS 证书包、RSA-3072 签名固件与 rollback 构建已通过。实机已完成不擦除 NVS 的 USB 迁移、四区 digest verify、`firmware_health_confirmed`、boot、serial-control、App 恢复与 BLE heartbeat；真实 OTA 下载、断电 rollback、App UI 完整点击安装和长时 soak 仍待验收。
...
`firmware-release.json` 与 `finalize-device.sh` 只记录 `0.9.6` 历史直刷基线，不是当前的生产分发入口。`0.10.0` 的 USB 迁移与升级后主链路 HIL 以 FF-7 证据页为准；真实 OTA 与 rollback 仍不得写为 PASS。新用户应使用 macOS App 的 USB 安装器，后续使用受签名发布清单约束的 OTA。
